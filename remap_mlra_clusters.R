...
mupolygon_lookup <- read.csv("Classified_NC_Traverses_uniqueID_mupolykey.csv")
...
for(watershed_number in 2:359){
  ...
  for(row in 2:nrow(current_watershed_mukeys)){
    ...
    number.of.polygons <- sum(!is.na(current_watershed_mukeys[row,-c(1:12, length(current_watershed_mukeys),length(current_watershed_mukeys)-1)])) 
  ...
  try(writeOGR(watershed_traverses, dsn= "B:/remapclusters", driver = "ESRI Shapefile", layer = paste0("pred",watershed_number)))
}
